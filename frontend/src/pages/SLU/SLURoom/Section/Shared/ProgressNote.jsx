...
import QuestionMark from '../../../ui/question-mark.png';
    ...
    currentIntent.length !== 0 && currentIntent[0][0] !== 'generic_intent' ? (
      <>
        <Grid container style={{ lineHeight: '40px' }}>
          ...
          {intentInfo.INTENT[currentIntent[0][1]].slot.map(
            (property, index) => {
                  ...
                  <Grid item xs={8}>
                    {currentIntentIndex !== -1 ? (
                      ...
                      property === 'four_last_digits' ? (
                        currentIntent[currentIntentIndex][1]
                      ) : property === 'city' ? (
                        showCity(currentIntent[currentIntentIndex][1])
                      ) : property === 'district' ? (
                        showDistrict(currentIntent[currentIntentIndex][1])
                      ) : property === 'city' || property === 'district' ? (
                        intentInfo[property.toUpperCase()][
                          currentIntent[currentIntentIndex][1]
                        ]
                      ) : (
                        intentInfo[
                          currentIntent[currentIntentIndex][0].toUpperCase()
                        ][currentIntent[currentIntentIndex][1]].name
                      )
                    ) : (
                      <img
                        src={QuestionMark}
